...
```math
\max_D \left( \⁡min_G ⁡(V(G,D) \right) 
```
where $V(G,D) = E_\left(P_\left( input \right) (x)\right) log⁡ D(x) + E_{P_model (x)} log⁡(1-D(x))$. <br />
